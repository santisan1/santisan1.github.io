...
let user_id = onAuthStateChanged(auth, async (user) => {
    ...
    let userNameR = document.querySelector("#register-nombre").value;
    let userNameIS= document.querySelector("#isname").value;
    checkInicio(user)
    ...
    if (userNameR){
        ...
        user.displayName = userNameR;
    }else if (userNameIS){
        console.log(userNameIS)
        user.displayName = userNameIS;
    }
